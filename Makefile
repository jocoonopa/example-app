...
REGISTRY ?= registry.gitlab.com/jocoonopa/example-app
...
docker-build:
	docker buildx build . --target cli -t ${REGISTRY}/cli:${VERSION}
	docker buildx build . --target cron -t ${REGISTRY}/cron:${VERSION}
	docker buildx build . --target fpm_server -t ${REGISTRY}/fpm_server:${VERSION}
	docker buildx build . --target web_server -t ${REGISTRY}/web_server:${VERSION}
